...
**延伸一下：上面我们都是在说update，那insert和delete呢？**
* 对于insert，只需要保存trx_id，该条记录对于其他事务，只有可见和不可见的区别，因此也不需要roll_pointer
* 对于delete，实际上还有一个隐藏列，会标记该条记录是否被删除，因此在可见和不可见的前提下，再判断下记录是否删除，得到最终结果
